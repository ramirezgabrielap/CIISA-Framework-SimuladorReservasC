...
// valida rut
$existe = $objreserva->SearchReserva($rut);
if($existe){
    echo "<div align='center'> Lo sentimos. <br><br> Ya existe una cabaña agendada para el rut ".$rut.". <br><br><button style='background-color: #A9DFBF' onclick='return window.history.back();'>VOLVER A INTENTAR</button></div>" . "<br>";
    return false;
} 
...
foreach($days_range as $day){
    $validafecha = $objreserva->ValidarFecha($day, $type);
...
$respuesta = $objreserva->InsertReserva($rut, $dv, $name, $lastname, $acom, $days, $date, $type);
echo "<br>" .$respuesta;
echo "<br><div style='display: grid; justify-content: center; align-items: center; background-color:  #D4EFDF'><b> RESUMEN DE SU RESERVA </b><br>" . "<br> Solicitante: " . $name . " " . $lastname . "<br> Rut Solicitante: " . $rut . "-" . $dv . "<br> Cantidad de Acompañantes: " . $acom . "<br> Días de Estadía: " . $days . "<br> A partir del: " . $date . "<br> En la Cabaña: " . $type . ", para " . $total_personas. " personas en total. </div>";
